...
class DailyController: ScrollingNavigationViewController, ScrollingNavigationControllerDelegate {

    let bag = DisposeBag()
    
    fileprivate let connectivity: Connectivity = Connectivity()
    private var viewModel: DailyJobInfoViewModel?
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "日结兼职"
        
        configureConnectivityNotifier()
        connectivity.startNotifier()
        
        // 初始化ViewModel
        viewModel = DailyJobInfoViewModel(
            input: (
            ...
            dependency: (
                disposeBag: bag,DailyJobNetworkService()))
...
private extension DailyController {
