...
/**
 * 设置为YES后，如果你的动画元素是基于`UIButton`映射出来的，动画元素的颜色将和其保持一致。
 * 同时取消`animatedHeightCoefficient`对动画元素高度的限制，即高度和原控件高度保持一致。
 */
@property (nonatomic, assign) BOOL autoSyncButtonBackgroundColor;
