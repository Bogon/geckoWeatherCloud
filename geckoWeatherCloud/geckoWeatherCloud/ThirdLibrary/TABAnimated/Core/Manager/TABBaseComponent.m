...
@property (nonatomic, assign) BOOL right_autoAlign;
@property (nonatomic, assign) BOOL bottom_autoAlign;
@property (nonatomic, assign) BOOL centerX_autoAlign;
@property (nonatomic, assign) BOOL centerY_autoAlign;
...
+ (instancetype)initWithComponentLayer:(TABComponentLayer *)layer
                      componentManager:(TABComponentManager *)componentManager {
    TABBaseComponent *component = [TABBaseComponent initWithComponentLayer:layer];
...
#pragma mark - Rather Methods

- (TABBaseComponentIntegerBlock)left_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.right_autoAlign = NO;
        
        self.layer.frame = CGRectMake(ratherLayer.layer.frame.origin.x, self.layer.frame.origin.y, self.layer.frame.size.width, self.layer.frame.size.height);
        return self;
    };
}

// 自动对齐问题
- (TABBaseComponentIntegerBlock)right_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.right_autoAlign = YES;
        
        CGFloat newX = ratherLayer.layer.frame.origin.x + ratherLayer.layer.frame.size.width - self.layer.frame.size.width;
        self.layer.frame = CGRectMake(newX, self.layer.frame.origin.y, self.layer.frame.size.width, self.layer.frame.size.height);
        return self;
    };
}

- (TABBaseComponentIntegerBlock)top_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.bottom_autoAlign = NO;
        
        self.layer.frame = CGRectMake(self.layer.frame.origin.x, ratherLayer.layer.frame.origin.y, self.layer.frame.size.width, self.layer.frame.size.height);
        return self;
    };
}

// 自动对齐问题
- (TABBaseComponentIntegerBlock)bottom_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.bottom_autoAlign = YES;
        
        CGFloat newY = ratherLayer.layer.frame.origin.y + ratherLayer.layer.frame.size.height - self.layer.frame.size.height;
        self.layer.frame = CGRectMake(self.layer.frame.origin.x, newY, self.layer.frame.size.width, self.layer.frame.size.height);
        return self;
    };
}

// 自动对齐问题
- (TABBaseComponentIntegerBlock)centerX_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        //TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.manager.animation(index);
        self.centerX_autoAlign = YES;
        self.right_autoAlign = NO;
...
// 自动对齐问题
- (TABBaseComponentIntegerBlock)centerY_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
        //TABBaseComponent *ratherLayer = self.manager.animation(index);
        self.manager.animation(index);
        self.centerY_autoAlign = YES;
        self.bottom_autoAlign = NO;
...
- (TABBaseComponentIntegerBlock)width_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
...
- (TABBaseComponentIntegerBlock)height_equalTo {
    return ^TABBaseComponent *(NSInteger index) {
